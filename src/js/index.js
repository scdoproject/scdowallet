...
function importAccounts(){
  ...
  dialog.showOpenDialog(
    ...
    if (obj.canceled){
      ...
      srcpath = obj.filePaths;
      dstpath = client.accountPath
...
function ToAccountInfo(publickey, balance, shard) {
    ...
    divhtml += `<li><span>` + balance + `</span> SCDO</li>`;
...
//     divhtml += `</dl>`;
    ...
    divhtml += `<dd class="lit" id="toTransfer">Transfer Scdo & Tokens</dd>`;
    divhtml += `</dl>`;
    divhtml += `<dl style="cursor: pointer;" onclick="viewOnScdoscan('` + publickey + `')">`;
    divhtml += `<dt><a href="#"><img src="./src/img/ViewonScdoscan.png"></a></dt>`;
    divhtml += `<dd class="lit" id="toView">View On Scdoscan</dd>`;
    divhtml += `</dl>`;
    ...
    divhtml += `<p class="info lit" id="createInfo">Accounts are password protected keys that can hold scdo. They can control contracts, but can't display incoming <span>transactions</span>.</p>`;
    ...
    // divhtml += `<span style="font-size:15px; font-family: sans-serif; text-align: center; "> Choose "IP:PORT"s from Scdoscan </span>`;
    // divhtml += `<input type="image" src="./src/img/ViewonScdoscan.png" alt="Submit" width="18" height="18" style="cursor: pointer;" onclick="viewOnScdoscan('` + publickey + `')">`;
  ...
  navigator.permissions.query({name: "clipboard-write"}).then(result => {
    if (result.state == "granted" || result.state == "prompt") {
      navigator.clipboard.writeText(text).then(
        ...
        const lang = document.getElementById("lang").value
        const copyMsg=MSGJSON[lang]["copySucess"]
        layer.msg(copyMsg)
      }, function() {
...
function startMining(publickey) {
    scdoClient.startMine(publickey).then((data)=>{
        console.log(data);
    }).catch((err)=>{
        console.log(err);
    });
...
function stopMining2(shard) {
    scdoClient.StartNode(shard).then(data=>{
        console.log(data);
    }).catch((err)=>{
        console.log(err);
    });
...
function stopMining(shard) {
    scdoClient.reStart(shard).then(data=>{
        console.log(data);
    }).catch((err)=>{
        console.log(err);
    });
...
function changeMingingStatus(publickey) {
    var shard = scdoClient.getShardNum(publickey);
    ...
    // scdoClient.killNonminingNodeProcess(shard);
}
