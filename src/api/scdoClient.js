...
function scdoClient() {
    ...
    this.configpath = os.homedir()+"/.ScdoWallet/viewconfig_1.1.json";
    this.accountPath = os.homedir() + "/.ScdoWallet/account/";
    this.rcPath = os.homedir() + "/.ScdoWallet/rc/";
    this.nodeConfigPath = os.homedir() + "/.ScdoWallet/node/";
    this.txPath = os.homedir() + "/.ScdoWallet/tx/";
    this.txArray = [];
    this.txRecords = [];
    this.keyTool = new scdojs().keyTool;
    this.init = function () {
        if (!fs.existsSync(os.homedir() + "/.ScdoWallet/")) {
            fs.mkdirSync(os.homedir() + "/.ScdoWallet/")
            fs.mkdirSync(this.accountPath)
        }

        if (!fs.existsSync(os.homedir()+'/.ScdoWallet')){
          fs.mkdirSync(os.homedir()+'/.ScdoWallet', { recursive: true }, (err) => {if (err) throw err;})
        }
        if (!fs.existsSync(os.homedir()+'/.ScdoWallet/tx')) {
          fs.mkdirSync(os.homedir()+'/.ScdoWallet/tx', { recursive: true }, (err) => {if (err) throw err;})
        }
        if (!fs.existsSync(os.homedir()+'/.ScdoWallet/rc')) {
          fs.mkdirSync(os.homedir()+'/.ScdoWallet/rc', { recursive: true }, (err) => {if (err) throw err;})
        }
        if (!fs.existsSync(os.homedir()+'/.ScdoWallet/account')) {
          fs.mkdirSync(os.homedir()+'/.ScdoWallet/account', { recursive: true }, (err) => {if (err) throw err;})
        }
        if (!fs.existsSync(os.homedir()+'/.ScdoWallet/viewconfig_1.1.json')) {
          console.log('not found, copy');
          var err = shell.cp('-f', `${__dirname}/../json/viewconfig_1.1.json`, os.homedir()+'/.ScdoWallet/')
          console.log(err)
        }
        if (!fs.existsSync(os.homedir()+'/.ScdoWallet/lang.json')) {
          var err = shell.cp('-f', `${__dirname}/../json/lang.json`, os.homedir()+'/.ScdoWallet/')
          console.log(err)
        }
    };
    this.init()
    ...
    this.client = [
      0,
      new scdojs(this.address[1]).client,
      new scdojs(this.address[2]).client,
      new scdojs(this.address[3]).client,
      new scdojs(this.address[4]).client
    ]
    ...
    this.startMine = function(publickey) {
        ...
        this.execute('echo ')
        // this.execute('/Users/scdo/go/src/github.com/scdoproject/ScdoWallet/cmd/mac/node start  -c /Users/scdo/.ScdoWallet/node/node-1.json')

        // this.execute('nodeexc start  -c /Users/scdo/.ScdoWallet/node/node-1.json')
    ...
    this.setUpNodeFile = function(dstfile, account, shard, initiate){
        ...
        file.set("basic.dataDir", 'ScdoWallet-node'+shard);
        file.set("ipcconfig.name", 'ScdoWallet'+shard+'.ipc');
        //p2p privatekey
        if(initiate === true) {
            var args = [
                'key',
        ...
        return new Q((resolve, reject) => {
            try {
                ...
                proc.stdout.on('data', data => {
                    ...
                    var privatekey = this.ParsePrivateKey(output)
                    var publickey = this.ParsePublicKey(output)
                    // scdoClient.makeNodeFile(publickey,shardnum)
    ...
    this.callContract = function (shard, payload, to, callBack) {
        // curl -H "Content-Type: application/json" -X POST --data '{"jsonrpc":"2.0","method":"scdo_call","params":["0x47a99059219055cf8277d5d7dff933446edb0012","0x6d4ce63c",-1],"id":1}' 117.50.97.136:8036
        // let hash = $('#QueryHash').text()
    ...
    // this.rcPath = os.homedir() + "/.ScdoWallet/rc/";
    this.rcPath = os.homedir() + path.sep + '.ScdoWallet' + path.sep + 'rc' + path.sep;
